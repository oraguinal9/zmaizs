...
function getServerBase(): string {
  if (typeof window === "undefined") return "";
  const saved = localStorage.getItem("gf_api_server") || "";
  if (saved) return saved;
  const origin = window.location.origin;
  if (origin.includes("localhost") || origin.includes("127.0.0.1")) {
    return "https://xiaoling.zbjh.top";
  }
  return origin;
}

export async function speakText(text: string, style: string = "yunxi"): Promise<void> {
  if (typeof window === "undefined") return;
  stopSpeaking();
  ttsSpeaking = true;
  const token = ++ttsCancelToken;
  const base = getServerBase();
  const ttsUrl = `${base}/tts?text=${encodeURIComponent(text)}&voice=${style}`;
...
export const VOICE_STYLES: Record<string, VoiceStyle> = {
  yunxi: { label: "云希", voice: "zh-CN-YunxiNeural", desc: "温暖磁性男友音" },
  yunfeng: { label: "云枫", voice: "zh-CN-YunfengNeural", desc: "清爽少年感" },
};
export function getVoiceStyle(key: string): VoiceStyle {
  return VOICE_STYLES[key] || VOICE_STYLES["yunxi"];
}
