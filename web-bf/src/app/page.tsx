...
export default function HomePage() {
  ...
  const [profile, setProfile] = useState<GirlProfile>({ name: "景琛", character: "金发酷帅，外表高冷内心温柔。说话低沉有磁性，偶尔霸道偶尔撒娇。会保护你、宠你、逗你笑。是让人心动的初恋男友类型。", birth: "5月20日", hobby: "健身、弹吉他、打篮球、喝咖啡", nickname: "宝贝" });
  ...
  // 语音
  const [voiceStyle, setVoiceStyle] = useState("yunxi");
  ...
  const voiceStyleRef = useRef("yunxi");

  const scrollRef = useRef<HTMLDivElement>(null);
  const retryTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const processingRef = useRef(false);
  const [modelPath, setModelPath] = useState("/models/l016/L016.model3.json");
  const [mounted, setMounted] = useState(false);
  const [remainingChats, setRemainingChats] = useState(50);

  // 嘴型动画：Live2D 无此 API，保留占位
  ...
  useEffect(() => {
    ...
    const savedStyle = localStorage.getItem("gf_voicestyle") || "yunxi";
    setVoiceStyle(savedStyle); voiceStyleRef.current = savedStyle;
    const savedModel = localStorage.getItem("gf_model") || "/models/l016/L016.model3.json";
  ...
  return (
    <div className="relative flex flex-col h-dvh overflow-hidden bg-slate-900 select-none">
      {/* 顶部栏 */}
      {!voiceMode && (
        <header className="shrink-0 bg-white/95 backdrop-blur border-b border-slate-700 px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-2">
            <div className="relative">
              <div className="w-8 h-8 rounded-full bg-indigo-900 flex items-center justify-center text-base heartbeat">🌟</div>
          ...
          <div className="flex gap-1">
            <a href="/景琛_桌面版.zip" download title="下载桌面悬浮版" className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center text-sm hover:bg-indigo-900 transition-colors">💻</a>
            <button onClick={() => setShowSettings(true)} className="w-7 h-7 rounded-full bg-slate-800 flex items-center justify-center text-sm">⚙️</button>
          </div>
        </header>
      )}
      ...
      <div className={`${voiceMode ? "fixed inset-0 z-0" : "relative w-full shrink-0"}`} style={voiceMode ? {} : { height: "50vh", maxHeight: "450px" }}>
        {mounted ? (
          <Live2DCharacter key={`${modelPath}-${voiceMode ? "v" : "t"}`} modelPath={modelPath} />
        ) : (
          <div className="w-full h-full bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-950 flex items-center justify-center">
        ...
        {voiceMode && subtitle && (
          <div className="absolute bottom-4 left-4 right-4 z-10 pointer-events-none">
            ...
            {!subtitleRole && <div className="flex justify-center"><div className={`px-4 py-2 rounded-full text-sm shadow-lg backdrop-blur ${isListening ? "bg-blue-600/90 text-white animate-pulse" : "bg-white/80 text-gray-400"}`}>{subtitle}</div></div>}
          </div>
        )}

        {/* 🎤 按钮 */}
        <div className="absolute top-3 right-3 z-10">
          <button onClick={toggleVoiceMode} className={`w-12 h-12 rounded-full flex items-center justify-center text-xl shadow-lg transition-all active:scale-90 ${voiceMode ? "bg-blue-600 text-white shadow-blue-300" : "bg-white/90 text-blue-600 shadow-blue-100"}`}
        ...
        {voiceMode && isMobileVoice && (
          <div className="absolute bottom-6 left-0 right-0 z-10 flex justify-center">
            <button
              ...
              className={`w-24 h-24 rounded-full flex items-center justify-center text-lg font-bold shadow-2xl transition-all select-none touch-none ${mobileRecording ? "bg-red-500 text-white scale-110 shadow-red-300" : "bg-white text-blue-600 shadow-blue-200 active:scale-95"}`}
        ...
        {loading && voiceMode && (
          <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
            <div className="flex gap-1.5"><span className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: "0s" }} /><span className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: "0.15s" }} /><span className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: "0.3s" }} /></div>
          </div>
        )}
      </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-2 space-y-2 chat-scroll">
            ...
            {messages.map((m) => (
              <div key={m.id} className={`msg-in flex ${m.role === "user" ? "justify-end" : "justify-start"} gap-2`}>
                {m.role === "assistant" && <div className="shrink-0 w-7 h-7 rounded-full bg-indigo-900 flex items-center justify-center text-xs self-end">🌟</div>}
            ...
            {loading && (
              <div className="flex items-start gap-2 msg-in">
                <div className="shrink-0 w-7 h-7 rounded-full bg-indigo-900 flex items-center justify-center text-xs">🌟</div>
                <div className="bg-white rounded-lg px-3 py-2 flex gap-1 items-center"><span className="w-1.5 h-1.5 bg-blue-300 rounded-full dot-1" /><span className="w-1.5 h-1.5 bg-blue-300 rounded-full dot-2" /><span className="w-1.5 h-1.5 bg-blue-300 rounded-full dot-3" /></div>
              </div>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex flex-col justify-end" onClick={(e) => { if (e.target === e.currentTarget) setShowSettings(false); }}>
          ...
          <div className="relative bg-white rounded-t-3xl max-h-[80vh] overflow-y-auto p-5 sheet-in space-y-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div>
              ...
              <input type="password" value={apiKey} onChange={(e) => setApiKey(e.target.value)} placeholder="sk-..." className="w-full mt-1 px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-blue-400" />
            </div>
            <div>
              <label className="text-sm font-medium text-gray-600">🌐 服务器地址（走代理，无需 Key）</label>
              <input type="text" value={serverUrl} onChange={(e) => setServerUrl(e.target.value)} placeholder="https://your-server.com" className="w-full mt-1 px-3 py-2.5 rounded-xl border border-gray-200 text-sm outline-none focus:border-blue-400" />
            </div>
              ...
              <div>
                ...
                <div className="flex gap-2 mt-1">
                  {Object.entries(VOICE_STYLES).map(([key, s]) => (
                    <button key={key} onClick={() => setVoiceStyle(key)} className={`flex-1 py-2 rounded-xl text-sm transition-all ${voiceStyle === key ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-600"}`}>{s.label}</button>
                  ))}
                </div>
              </div>
            )}
            ...
            <div className="space-y-3">
              <p className="text-sm font-medium text-gray-600">👤 男友人设</p>
              <div className="grid grid-cols-2 gap-3">
                {[{ k: "name", label: "名字", ph: "景琛" }, { k: "birth", label: "生日", ph: "5月20日" }, { k: "nickname", label: "叫你", ph: "宝贝" }, { k: "hobby", label: "爱好", ph: "健身、弹吉他" }].map((f) => (
                  <div key={f.k}><label className="text-xs text-gray-400">{f.label}</label><input value={(profile as any)[f.k]} onChange={(e) => setProfile({ ...profile, [f.k]: e.target.value })} placeholder={f.ph} className="w-full mt-0.5 px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-400" /></div>
                ))}
              </div>
              <div><label className="text-xs text-gray-400">性格</label><input value={profile.character} onChange={(e) => setProfile({ ...profile, character: e.target.value })} placeholder="金发酷帅，外表高冷内心温柔" className="w-full mt-0.5 px-3 py-2 rounded-lg border border-gray-200 text-sm outline-none focus:border-blue-400" /></div>
            </div>

            <div className="flex gap-3 pt-1">
              <button onClick={saveSettings} className="flex-1 py-3 bg-blue-600 text-white rounded-xl font-medium text-sm active:scale-95">保存设置</button>
            ...
            <p className="text-center text-xs text-gray-300 pt-1">景琛 AI男友 v1.0</p>
          </div>
        </div>
      )}
    </div>
